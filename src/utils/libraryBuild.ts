...
const normalizeCategoryName = (value: string) =>
  value.trim() === '' ? '(Uncategorized)' : value.trim();
...
export const buildLibraryLayout = ({
  ...
  Array.from(categoryMap.entries())
    ...
    .forEach(([category, bookIds]) => {
      ...
      const bookcaseId = previous?.id ?? createStableId('bookcase', `${libraryId}:${category}`);
...
export const rebuildStateFromCsv = ({
  ...
  const activeLibraryId =
    ...
    libraries.some((library) => library.id === previousState.activeLibraryId)
      ? previousState.activeLibraryId
      : libraries[0]?.id ?? '';
  return {
    booksById,
